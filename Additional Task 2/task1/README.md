...
| Query Description                                                 | HTTP Verb | Url                        |
| ----------------------------------------------------------------- | --------- | -------------------------- |
| Get phone number for a customer with “ANATR” id                   | GET       | /Customers('ANATR')/ Phone/$value |
| Get all customers from Germany                                    | GET       | /Customers?$filter=Country eq 'Germany' |
| Get the name of the company and the country in which it is located| GET       | /Customers?$select=CompanyName, Country |
| Get orders that have been delivered to Germany and the freight is more than 500| GET       | /Orders?$filter=ShipCountry eq 'Germany' and Freight gt 500|
| Get City and Freight of order which freight is more than 500      | GET       | /Orders?$filter=Freight gt 500&$select=ShipCity, Freight |
...
```
C:\Program Files (x86)\Microsoft Visual Studio\2019\Community>datasvcutil /dataservicecollection /in:c:\northwind-data-service.edmx /out:c:\NorthwindDataService3.cs
```
...
8. Найдите базовый класс, от которого унаследован _NorthwindModel.NorthwindEntities_. 
    ...
    * По какому пути лежит эта сборка? C:\Program Files (x86)\Microsoft WCF Data Services\5.0\bin\.NETFramework\ Microsoft.Data.Services.Client.dll
...
| ---------- | ----------- | ----------------- | ----------- |
| #1.1       |14952	       |1	                 |Main Thread  |
| #1.2       |8000         |7                  |Worker Thread|
| #1.3       |14952	       |1	                 |Main Thread  |
| #2.1       |15692        |1	                 |Main Thread  |
| #2.2       |20156        |10                 |Worker Thread|
| #2.3       |20156        |10                 |Worker Thread|
